...
\hfil  \includegraphics[width=\textwidth]{M1DUThCombined.pdf} \hfil 
% the following blank line is important

\figcaption{MJDSimArticle}{A simulation of the efficiency of the Module 1 array of Ge detectors for detecting thorium contamination in the copper detector unit pieces. The black line shows all energy depositions recorded by the array that do not occur in the dead layer of a detector. The blue line has removed events occurring in multiple detectors, and the red line has additionally removed events that are detectable as multi-site within a single detector.}
...
\hfil  \includegraphics[width=\textwidth]{peakshape.png} \hfil 

\figcaption{MJDSimArticle}{A 2614~keV \iso{208}{Th} calibration peak fit to a Gaussian, high- and low-energy ExGaussian tails, a step function, and a low-order polynomial background.}
...
\indent Guinn has also implemented a sophisticated algorithm for \MJ\ that fits peaks in the \MJDemo\ energy spectrum to an analytic peak shape function (see \figref{MJDSimArticle}{peakshape} for an example). The peak shape function is composed of a Gaussian, high- and low-energy ExGaussian tails, a step function, and a low-order polynomial background. The peak fitter is currently being used to calibrate the energy spectrum of the detectors from a \iso{228}{Th} line source. Guinn is currently working on extending the peak fitter to fit multiple peaks simultaneously, which will allow for an automated and more robust energy calibration.

%\begin{wrapfigure}{l}{0.5\textwidth}
%\begin{center}
%\hfil  \includegraphics[width=.48\textwidth]{peakshape.png} \hfil 
%% the following blank line is important
%
%\end{center}
%\refstepcounter{figure} \hfil \parbox{.5\textwidth}{\small Figure~\ref{MJDSimArticle}-\arabic{figure}. A 2614~keV \iso{208}{Th} calibration peak fit to a gaussian, low-energy tail, and step function. }  \hfil
%
%\label{peakshape}  %put label{} after \figcaption{}
%
%\end{wrapfigure}
